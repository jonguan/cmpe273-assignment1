...
object User {
  ...
  def save(user: User): User = {
      ...
      if (user_id == -1) {
          // New User
          val new_id:Int = scala.util.Random.nextInt(10)
  ...
  def createCard(user_id:Int, card:IDCard): IDCard = {
      ...
      if (card_id == -1) {
          // New User
          val newCard_id = scala.util.Random.nextInt(10)
  ...
  // Web Login
    def createLogin(user_id:Int, login:WebLogin): WebLogin = {
      ...
      if (login_id == -1) {
          // New User
          val newLogin_id = scala.util.Random.nextInt(10)
  ...
  def createBankAccount(user_id:Int, account:BankAccount): BankAccount = {
      ...
      if (ba_id == -1) {
          // New User
          val newBa_id = scala.util.Random.nextInt(10)
